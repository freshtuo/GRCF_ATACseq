...
Merge consensus peaks from different groups, extract peak counts matrix and perform differential binding analysis.
...
# infer group id
peak.info <- data.frame(Condition=sapply(basename(peakfiles), FUN=function(x) { strsplit(x, '[.]')[[1]][1] }),
...
samples <- as.data.frame(sample.info %>% left_join(bam.info, by='SampleID') %>% left_join(peak.info, by='Condition'))
...
samples.peak <- peak.info
samples.peak$SampleID <- samples.peak$Condition

# create a dba object
peaks <- dba(minOverlap=1, sampleSheet=samples.peak)

# make a venn diagram
dba.plotVenn(peaks, peaks$mask$All)

# free space
rm(samples.peak, peaks)
```

### Read in peaksets, generate a consensus peakset, and count reads 

- Set up an experiment to analyze with the sample sheet.
- Make a consensus peakset by taking a union of all peaksets.
...
consensus.peaks <- dba.peakset(panc, bRetrieve=TRUE)

# apply ENCODE blacklist
panc <- dba.blacklist(panc, blacklist=DBA_BLACKLIST_GRCH37, greylist=FALSE)
